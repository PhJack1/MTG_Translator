...
### 🤖 자동 번역
확장 프로그램은 이제 수동 작업 없이 MTG 카드를 자동으로 번역할 수 있습니다. 팝업에서 "자동 번역" 확인란을 선택하고 대상 언어를 선택하기만 하면 됩니다. 지원되는 모든 페이지가 로드될 때 자동으로 번역이 실행되며 DOM 동적 변경도 감지합니다(Moxfield와 같은 실시간 편집 사이트).
